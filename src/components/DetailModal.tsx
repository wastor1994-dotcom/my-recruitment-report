...
function groupByPosition(rows: RateRequestRow[]) {
  ...
  for (const row of rows) {
    ...
    const officer = row.officer.trim() || "(ไม่ระบุ)";
...
export function DetailModal({ title, rows, onClose }: DetailModalProps) {
  ...
  return (
    ...
    >
      <div
        className="flex max-h-[90vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl border-2 border-red-200 bg-white shadow-2xl"
      ...
      >
        <div className="flex items-start justify-between gap-4 border-b border-red-100 bg-red-50 px-5 py-4">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-600">
              {rows.length.toLocaleString("th-TH")} ใบขอ — {positionGroups.length.toLocaleString("th-TH")} ตำแหน่ง
            </p>
          </div>
          <button
            type="button"
        ...
        {rows.length > 0 ? (
          <div className="border-b border-red-100 bg-amber-50/40 px-5 py-4">
              ...
              <tbody>
                {rows.map((row) => (
                  <tr key={row.id} className="border-b border-red-50 hover:bg-red-50/40">
                    ...
                    <td className="px-4 py-2.5 font-medium text-red-800">{row.officer || "—"}</td>
